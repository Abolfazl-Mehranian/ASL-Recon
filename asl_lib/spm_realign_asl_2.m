function P = spm_realign_asl_2(P,flags)
% This code is modified from the spm_realign.m from the SPM package. The modification is to remove the artifical motion component caused by the systematical label/control
% signal modulations in arterial spin labeled perfusion MRI.
% Ref: 1. Ze Wang, Improving cerebral blood flow quantification for arterial spin labeled perfusion MRI by removing residual motion artifacts and global signal fluctuations, 
%         Magnetic Resonance Imaging, 2012 (in press). http://dx.doi.org/10.1016/j.mri.2012.05.004
% 2. Ze Wang, Geoffrey Aguirre, Hengyi Rao, JiongJ Wang, Anna R. Childress, John A. Detre, Empirical optimization of ASL data analysis using an ASL data processing toolbox: ASLtbx, Magnetic Resonance Imaging, 2008, 26(2):261-9 
% Modification made by Ze Wang, 2010 at Upenn. 
% all rights reserved by Upenn regarding this modification.
% Estimation of within modality rigid body movement parameters
% FORMAT P = spm_realign_asl(P,flags)
%
...
%[pth,nm,xt,vr] = fileparts(deblank(PI));    % this line will cause errors for Matlab>7.11. Noticed by D.G. McLaren
% PO             = fullfile(pth,[pre nm xt vr]);
% PO =  fullfile(spm_str_manip(PI,'h'), [pre spm_str_manip(PI,'t')]);
[pth,nm,xt,vr] = spm_fileparts(deblank(PI));    % suggested by Chris Rorden 8/2013
PO             = fullfile(pth,[pre nm xt vr]);
return;
%_______________________________________________________________________
