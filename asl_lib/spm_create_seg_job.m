...
function job = spm_create_seg_job(io)
%-----------------------------------------------------------------------
% Job saved on 27-Jul-2018 15:46:20 by cfg_util (rev $Rev: 6460 $)
% spm SPM - SPM12 (6906)
% cfg_basicio BasicIO - Unknown
%-----------------------------------------------------------------------
job = [io.asl_lib_path 'asl_lib' os_bar 'spm_segmentation.m'];
fid = fopen(job,'w');
tmp = [''''  io.t1mprage.nii_flname ',1''};'];
...
tmp = ['''' io.spm_path 'tpm' os_bar 'TPM.nii,1''};'];
...
tmp = ['''' io.spm_path 'tpm' os_bar 'TPM.nii,2''};'];
...
tmp = ['''' io.spm_path 'tpm' os_bar 'TPM.nii,3''};'];
...
tmp = ['''' io.spm_path 'tpm' os_bar 'TPM.nii,4''};'];
...
tmp = ['''' io.spm_path 'tpm' os_bar 'TPM.nii,5''};'];
...
tmp = ['''' io.spm_path 'tpm' os_bar 'TPM.nii,6''};'];
